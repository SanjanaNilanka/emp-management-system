...
export default function AddEmployee() {
  ...
  const handleSubmit = async (event) => {
    ...
    console.log({
      ...
      empID: data.get('emp-id'),
      nic: data.get('nic'),
      email: data.get('email'),
      address: data.get('address'),
      phone: data.get('phone'),
      workExperience: data.get('work-experience'),
      marriageStates: data.get('marriage-states'),
    ...
    try {
      const response = await axios.post(`http://localhost:8000/employee/create`, empData);
